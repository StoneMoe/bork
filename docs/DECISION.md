...
- Bork 可尝试同时 UDP 打洞。
- 对 Tracker 返回的 IPv4 候选，首次发现时使用同一个 UDP 端点向观测端口两侧各 16 个端口发送有房间准入 MAC 的 Hello probe，以覆盖常见的顺序端口分配型对称 NAT；扫描窗口固定且有界，不保存为长期发现候选，也不用于 IPv6。
- 网关不支持 PCP、NAT-PMP 或 UPnP 不等于房间连接失败；STUN 已获得映射时继续使用 Tracker 协调 UDP 打洞，界面不显示可选网关映射告警，但诊断快照保留原始失败原因。自动映射与 STUN 均不可用时才显示端口映射告警。
