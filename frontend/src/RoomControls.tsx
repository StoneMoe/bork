...
import { locale, t, translateMessage } from "./i18n";
...
export function RoomControlRow(props: RoomControlRowProps) {
  ...
  return (
    <>
      <div class="room-control-row" role="group" aria-label={t("房间控制")}>
        <div class="room-feature-bar" role="group" aria-label={t("房间功能")}>
          <button
            class="feature-button"
            classList={{ active: props.screenSharing }}
            type="button"
            disabled={props.busy || !props.ready || props.captureBusy}
            aria-label={props.screenSharing ? t("停止屏幕分享") : props.captureBusy ? t("选择要分享的屏幕") : t("开始屏幕分享")}
            aria-pressed={props.screenSharing}
            title={props.screenSharing ? t("停止屏幕分享") : t("开始屏幕分享")}
            onClick={props.toggleScreenShare}
          >
            <ScreenShareIcon />
            <span>
              <strong>{props.screenSharing ? t("停止分享") : props.captureBusy ? t("选择屏幕…") : t("屏幕分享")}</strong>
              <small>{props.remoteSharerCount ? t("{count} 人正在分享", { count: props.remoteSharerCount.toLocaleString(locale()) }) : props.screenSharing ? t("你的画面正在共享") : t("选择窗口或屏幕")}</small>
            </span>
          </button>
          <button
            ref={fileShareButton}
            ...
            aria-label={incomingFile() ? t("打开文件分享，有待接收文件") : t("打开文件分享")}
          ...
          >
            <FileShareIcon />
            <span>
              <strong>{t("文件分享")}</strong>
              <small>{props.state.room?.transfers.length ? t("{count} 条记录", { count: props.state.room.transfers.length.toLocaleString(locale()) }) : t("选择成员发送")}</small>
            </span>
          </button>
        </div>
        <div class="voice-controls" role="group" aria-label={t("语音控制")}>
          <AudioControl
            kind="capture"
            label={t("麦克风")}
...
export function RoomMemberList(props: { state: AppState; remotePeers: RemotePeer[]; focusFallback: () => void }) {
  ...
  const localName = () => props.state.nickname || props.state.room?.peerId.slice(0, 14) || t("佚名");
  ...
  const memberStatus = (captureMuted: boolean, playbackMuted: boolean, screenSharing: boolean) => [
    captureMuted ? t("麦克风已静音") : "",
    playbackMuted ? t("扬声器已静音") : "",
    screenSharing ? t("分享屏幕") : "",
  ].filter(Boolean).join(" · ");
  const remoteStatus = (remotePeer: RemotePeer) => remotePeer.connected ? memberStatus(remotePeer.muted, remotePeer.playbackMuted, remotePeer.screenSharing) || t("在线") : t("恢复连接中");
  const remoteTransport = (remotePeer: RemotePeer) => !remotePeer.connected ? t("恢复连接中") : remotePeer.transport === "bridge" ? t("桥接") : t("直连");
  ...
  return (
    <>
      <div ref={memberList} class="member-list" role="list" aria-label={t("当前房间成员")}>
          ...
          <span class="member-identity">
            <span class="member-name-line">
              <button
                ...
                type="button"
                aria-label={t("查看 {name} 的详细信息", { name: localName() })}
              ...
              <Show when={props.state.audio.captureMuted}>
                <span class="member-muted-icon" role="img" aria-label={t("麦克风已静音")} title={t("麦克风已静音")}><MicrophoneIcon muted /></span>
              </Show>
              <Show when={props.state.audio.playbackMuted}>
                <span class="member-muted-icon" role="img" aria-label={t("扬声器已静音")} title={t("扬声器已静音")}><SpeakerIcon muted /></span>
              </Show>
              <Show when={props.state.room?.screenSharing}>
                <span class="member-status-icon" role="img" aria-label={t("正在分享屏幕")} title={t("正在分享屏幕")}><ScreenShareIcon /></span>
              </Show>
            </span>
            <span class="visually-hidden">{localSpeaking() ? t("正在说话") : t("未在说话")}</span>
          </span>
          <span class="member-network">
            <strong class="member-connection local">{t("你")}</strong>
          </span>
        </div>
              ...
              <span class="member-identity">
                <span class="member-name-line">
                  <button
                    class="member-name member-name-trigger"
                    type="button"
                    aria-label={t("查看 {name} 的详细信息", { name: remoteName(peer()) })}
                    aria-controls={memberInfoPopoverID}
                ...
                <span class="visually-hidden">{remoteSpeaking(peer()) ? t("正在说话") : t("未在说话")}</span>
              </span>
              <span class="member-network">
                ...
                <Show when={peer().connected}>
                  <small class="member-latency">{(peer().rttMillis || 1).toLocaleString(locale())} ms</small>
                </Show>
              </span>
            </div>
          );
      ...
      >
        <Show when={selectedMember() === "local"} fallback={selectedRemote() && (
          <div class="member-info-grid">
            <span><small>{t("昵称")}</small><b>{remoteName(selectedRemote()!)}</b></span>
            <span><small>Session</small><code>{selectedRemote()!.sessionId || t("未知")}</code></span>
            <span><small>{t("连接")}</small><b>{remoteTransport(selectedRemote()!)}{selectedRemote()!.connected ? ` · ${(selectedRemote()!.rttMillis || 1).toLocaleString(locale())} ms` : ""}</b></span>
            <span><small>{selectedRemote()!.transport === "bridge" ? t("下一跳") : t("远端地址")}</small><code>{selectedRemote()!.address}</code></span>
            <span><small>{t("状态")}</small><b>{remoteStatus(selectedRemote()!)}</b></span>
          </div>
        )}>
          <div class="member-info-grid">
            <span><small>{t("昵称")}</small><b>{props.state.nickname || t("本机")}</b></span>
            <span><small>{t("本机端点")}</small><code>{props.state.diagnostics.listenAddress || t("尚未打开")}</code></span>
          </div>
        </Show>
      </aside>
    </>
...
function FileSharePopover(props: {
  ...
  return (
    ...
    >
      <header>
        <div><strong id="file-share-title">{t("发送文件")}</strong><small id="file-share-description">{t("选择一个已认证成员，然后选择本机文件。")}</small></div>
        <button
      ...
      <div class="file-recipient-list">
        <Show when={props.remotePeers.length > 0} fallback={<p class="file-recipient-empty">{t("正在等待可发送文件的房间成员。")}</p>}>
          <For each={props.remotePeers.map((peer) => peer.peerId)}>{(peerID) => {
            const peer = () => props.remotePeers.find((candidate) => candidate.peerId === peerID)!;
            return (
              <button
                ...
                onClick={async (event) => {
                  const button = event.currentTarget;
                  await runFileAction(async () => { await Backend.OfferFile(peerID, t("选择要发送的文件")); });
                  queueMicrotask(() => {
                    const active = document.activeElement;
                    if (active === button) {
              ...
              >
                <span><strong>{remoteName(peer())}</strong><small>{peer().connected ? `${peer().transport === "bridge" ? t("桥接") : t("直连")} · ${(peer().rttMillis || 1).toLocaleString(locale())} ms` : t("恢复连接中")}</small></span>
                <b>{peer().connected ? t("选择文件") : t("暂不可用")}</b>
              </button>
            );
          }}</For>
        </Show>
      ...
      <Show when={actionError()}>
        <p class="file-action-error" role="alert">{translateMessage(actionError())}</p>
      </Show>
      <TransferPanel
        state={props.state}
        busy={props.busy}
...
function TransferPanel(props: { state: AppState; busy: boolean; runAction: (action: () => Promise<void>) => Promise<boolean>; focusFallback: () => void }) {
  ...
  const localName = () => props.state.nickname || t("本机");
  const peerName = (transfer: FileTransfer) => transfer.peerNickname || transfer.peerId.slice(0, 14);
  const status = (transfer: FileTransfer) => ({
    preparing: t("正在准备"), offered: transfer.direction === "incoming" ? t("等待接收") : t("等待对方接收"),
    accepting: t("正在创建文件"), transferring: t("传输中"), waiting: t("等待确认"),
    completed: t("已完成"), rejected: t("已拒绝"), canceled: t("已取消"), failed: t("失败"),
  ...
  return (
    <Show when={transfers().length > 0}>
      <section class="transfer-panel" aria-label={t("文件传输")}>
        <header><strong>{t("文件传输")}</strong><span>{transfers().length.toLocaleString(locale())}</span></header>
          ...
          return (
            <article class="transfer-row" classList={{ failed: transfer().status === "failed" }}>
              ...
              <div class="transfer-progress">
                <progress value={progressValue()} max={progressMax()} aria-label={t("{name} 传输进度", { name: transfer().name })} />
                <span>{formatBytes(transfer().transferred)} / {formatBytes(transfer().size)}</span>
              </div>
              <div class="transfer-state">
                <b>{status(transfer())}</b>
                <Show when={transfer().error}>{(error) => <small title={translateMessage(error())}>{translateMessage(error())}</small>}</Show>
              ...
              <div class="transfer-actions">
                <Show when={incoming() && transfer().status === "offered"}>
                  <button type="button" disabled={props.busy} onClick={(event) => void runTransferAction(event.currentTarget, () => Backend.AcceptFile(transferID, t("保存接收的文件")))}>{t("接收")}</button>
                  <button type="button" disabled={props.busy} onClick={(event) => void runTransferAction(event.currentTarget, () => Backend.RejectFile(transferID))}>{t("拒绝")}</button>
                </Show>
                <Show when={!terminal(transfer()) && !(incoming() && transfer().status === "offered")}>
                  <button type="button" disabled={props.busy} onClick={(event) => void runTransferAction(event.currentTarget, () => Backend.CancelFile(transferID))}>{t("取消")}</button>
                </Show>
              </div>
            </article>
          );
...
function AudioControl(props: AudioControlProps) {
  ...
  const actionLabel = () => {
    if (props.pushToTalk) return props.muted ? t("按住设定按键说话；点击切换到开放麦克风") : t("正在按键说话；点击切换到开放麦克风");
    return props.muted ? t("取消{device}静音", { device: props.label }) : t("将{device}静音", { device: props.label });
  };
  const levelPercent = () => {
  ...
  return (
    <div class="audio-control">
      <button
        ...
        title={props.clipped ? t("麦克风输入削波 · {action}", { action: actionLabel() }) : actionLabel()}
      ...
      <Show when={props.kind === "capture"}>
        <span
          ...
          role="meter"
          aria-label={t("麦克风输入电平")}
          aria-valuemin="0"
          aria-valuemax="100"
          aria-valuenow={Math.round(levelPercent())}
          aria-valuetext={props.clipped ? t("输入削波") : `${Math.round(levelPercent()).toLocaleString(locale())}%`}
        >
          <span style={{ transform: `scaleX(${levelPercent() / 100})` }} />
        </span>
        <span class="visually-hidden" role="status" aria-live="polite">
          {props.clipped ? t("麦克风输入削波") : ""}
        </span>
      </Show>
      <div class="gain-slider">
        <input
          ...
          value={draftGain()}
          disabled={props.disabled}
          aria-label={t("{device}音量", { device: props.label })}
          title={t("{device}音量 {gain}%", { device: props.label, gain: draftGain().toLocaleString(locale()) })}
          onInput={(event) => { editingGain = true; setDraftGain(event.currentTarget.valueAsNumber); }}
          onChange={() => void commitGain()}
        />
        <output>{draftGain().toLocaleString(locale())}%</output>
      </div>
    </div>
  );
}
...
function formatBytes(value: number): string {
  ...
  const format = new Intl.NumberFormat(locale(), { minimumFractionDigits: 1, maximumFractionDigits: 1 }).format;
  if (value < 1024 * 1024) return `${format(value / 1024)} KiB`;
  if (value < 1024 * 1024 * 1024) return `${format(value / 1024 / 1024)} MiB`;
