...
func TestLoadAppConfigFileLoadsLocalDirectoriesAndNormalizesNode(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yml")
	...
	want := GameProxyConfig{
		Directories: []string{filepath.Clean("/local/games"), filepath.Clean("/other/games")},
		Node: GameProxyNodeConfig{
			Server:   "proxy.example.com",
			Port:     4568,
			Username: "player",
...
func TestLoadAppConfigFileRejectsLegacyDirectoryField(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yml")
	if err := os.WriteFile(path, []byte("game_proxy:\n  directory: /local/games\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	if _, err := loadAppConfigFile(path); err == nil || !strings.Contains(err.Error(), "field directory not found") {
		t.Fatalf("load error = %v, want rejected legacy directory field", err)
	}
}
...
func TestValidateGameProxyNodeRejectsInvalidActivationValues(t *testing.T) {
	tests := []struct {
		...
		{"ipv6 server", func(node *GameProxyNodeConfig) { node.Server = "2001:db8::1" }},
		...
		{"mtu below range", func(node *GameProxyNodeConfig) { node.MTU = 67 }},
...
func TestGameProxyExportNodeBase64ContainsExactlySixTypedKeys(t *testing.T) {
	config := GameProxyConfig{Directories: []string{"/must/stay/local"}, Node: validGameProxyNode()}
...
func TestGameProxyExportNodeBase64IncludesEnabledEncryption(t *testing.T) {
	node := validGameProxyNode()
	node.Encrypt = true
	encoded, err := (GameProxyConfig{Node: node}).ExportNodeBase64()
	if err != nil {
		t.Fatal(err)
	}
	contents, err := base64.StdEncoding.DecodeString(encoded)
	...
	if len(fields) != 7 || string(fields["encrypt"]) != "true" {
		t.Fatalf("exported fields = %s", contents)
	}
}

func TestGameProxyImportNodeBase64ReplacesOnlyNode(t *testing.T) {
	config := GameProxyConfig{Directories: []string{"/local/games"}, Node: validGameProxyNode()}
	encoded := encodeNodeJSON(`{"server":"new.example.com","port":1234,"username":"new-user","password":" new-pass ","mtu":1280,"dns":"8.8.8.8","encrypt":true}`)

	if err := config.ImportNodeBase64(encoded); err != nil {
		t.Fatal(err)
	}

	if !reflect.DeepEqual(config.Directories, []string{"/local/games"}) {
		t.Fatalf("Directories = %q", config.Directories)
	}
	if config.Node.Server != "new.example.com" || config.Node.Password != " new-pass " || config.Node.Port != 1234 || !config.Node.Encrypt {
