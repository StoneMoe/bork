...
type fakeBridge struct {
	...
	startErr   error
	startHook  func(intercept.Callbacks)
...
func (bridge *fakeBridge) Start(ctx context.Context, callbacks intercept.Callbacks) error {
	...
	bridge.mu.Unlock()
	if bridge.startHook != nil {
		bridge.startHook(callbacks)
	}
...
func (bridge *fakeBridge) reportEndpointError(err error) {
	bridge.mu.Lock()
...
func (bridge *fakeBridge) UpdateRules(_ context.Context, paths []string) error {
	bridge.mu.Lock()
	defer bridge.mu.Unlock()
	bridge.updated = append([]string(nil), paths...)
	return bridge.updateErr
}

func (bridge *fakeBridge) updatedPaths() []string {
	bridge.mu.Lock()
	defer bridge.mu.Unlock()
	return append([]string(nil), bridge.updated...)
}
...
type fakeSupervisor struct {
	...
	mu              sync.Mutex
	status          iwan.Status
	dataPathEvents  []iwan.DataPathEvent
...
func (supervisor *fakeSupervisor) DrainDataPathEvents() (iwan.Status, []iwan.DataPathEvent, uint64) {
	supervisor.mu.Lock()
	defer supervisor.mu.Unlock()
	status := supervisor.status
	events := append([]iwan.DataPathEvent(nil), supervisor.dataPathEvents...)
...
func (supervisor *fakeSupervisor) appendDataPathEvent(event iwan.DataPathEvent) {
	supervisor.mu.Lock()
...
func validStartInput() StartInput {
	return StartInput{
		...
		Directories: []string{"games"}, DNS: netip.MustParseAddr("1.1.1.1"),
	}
}
