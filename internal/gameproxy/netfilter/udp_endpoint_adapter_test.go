...
func TestUDPEndpoint_WriteDatagram_preserves_source_and_reports_backend_error(t *testing.T) {
	...
	options := testNativeUDPOptions()
	endpoint := newUDPEndpoint(context.Background(), backend, metadata, options)
	...
	if len(calls) != 1 || calls[0].id != metadata.NativeID || calls[0].source != metadata.OriginalRemote ||
		string(calls[0].payload) != "response" || calls[0].options.flags != options.flags ||
		!reflect.DeepEqual(calls[0].options.data, options.data) {
		t.Fatalf("PostUDPReceive calls = %+v, want exact ID, source, payload, and options", calls)
	}
}

func TestUDPEndpoint_WriteDatagram_uses_latest_send_options(t *testing.T) {
	backend := &udpTestBackend{}
	metadata := udpTestMetadata(306)
	initial := testNativeUDPOptions()
	latest := nativeUDPOptions{flags: 0x40, data: []byte{9, 8, 7}}
	endpoint := newUDPEndpoint(context.Background(), backend, metadata, initial)
	if err := endpoint.enqueue(metadata.OriginalRemote, []byte("request"), latest); err != nil {
		t.Fatal(err)
	}
	request, err := endpoint.ReadDatagram(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	request.Payload = []byte("response")
	if err := endpoint.WriteDatagram(context.Background(), request); err != nil {
		t.Fatal(err)
	}

	calls := backend.receiveSnapshot()
	if len(calls) != 1 || calls[0].options.flags != latest.flags ||
		!reflect.DeepEqual(calls[0].options.data, latest.data) {
		t.Fatalf("PostUDPReceive options = %+v, want latest %+v", calls, latest)
	}
}
...
func TestUDPEndpoint_WriteDatagram_rejects_changed_identity_local_generation_and_invalid_source(t *testing.T) {
	tests := []struct {
		...
		{name: "native rule match", mutate: func(metadata *intercept.Metadata) { metadata.NativeRuleMatched = false }},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			endpoint := newUDPEndpoint(context.Background(), backend, metadata, testNativeUDPOptions())
...
func TestUDPEndpoint_WriteDatagram_serializes_writers(t *testing.T) {
	...
	var once sync.Once
	backend := &udpTestBackend{}
	backend.postHook = func(_ context.Context, _ intercept.NativeID, _ netip.AddrPort, payload []byte, _ nativeUDPOptions) error {
		if payload[0] == 'a' {
			once.Do(func() {
				close(firstEntered)
				<-releaseFirst
...
func TestUDPEndpoint_operations_respond_to_context_and_close(t *testing.T) {
	...
	backend.postHook = func(ctx context.Context, _ intercept.NativeID, _ netip.AddrPort, _ []byte, _ nativeUDPOptions) error {
		<-ctx.Done()
...
func TestUDPEndpoint_Reset_then_Close_suspends_once_and_exposes_cause(t *testing.T) {
	backend := &udpTestBackend{}
	endpoint := newUDPEndpoint(context.Background(), backend, udpTestMetadata(305), testNativeUDPOptions())
...
func udpTestMetadata(id intercept.NativeID) intercept.Metadata {
	return intercept.Metadata{
		Generation: 8, NativeID: id, ProcessID: 51, ExecutablePath: `c:\games\game.exe`, NativeRuleMatched: true,
