...
func (*tcpTestBackend) UpdateRules(context.Context, []nativeRule) error { return nil }
...
func (*fakeNativeBackend) PostUDPReceive(
	context.Context,
	intercept.NativeID,
	netip.AddrPort,
	[]byte,
...
func (backend *fakeNativeBackend) SuspendUDP(id intercept.NativeID) error {
	backend.mu.Lock()
	defer backend.mu.Unlock()
	backend.suspended = append(backend.suspended, id)
	return nil
}

func (*tcpTestBackend) PostUDPReceive(
	context.Context,
	intercept.NativeID,
	netip.AddrPort,
	[]byte,
...
func (callbacks *tcpTestCallbacks) EndpointError(err error) {
	if callbacks.endpointErrors != nil {
		callbacks.endpointErrors <- err
	}
}
