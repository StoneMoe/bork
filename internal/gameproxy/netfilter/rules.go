...
func exactRules(executablePaths []string) ([]nativeRule, error) {
	...
	rules := make([]nativeRule, 0, len(paths)*2+1)
	rules = append(rules, nativeRule{
		direction: nativeDirectionOutbound, family: nativeAddressFamilyIPv4,
		protocol: nativeProtocolUDP, flags: nativeFlagAllow, processID: intercept.ProcessID(os.Getpid()),
	})
	for _, path := range paths {
		rules = append(rules,
			nativeRule{
				direction: nativeDirectionOutbound, family: nativeAddressFamilyIPv4,
				protocol: nativeProtocolTCP, flags: nativeFlagIndicateConnectRequests,
				executablePath: path,
			},
			nativeRule{
				direction: nativeDirectionOutbound, family: nativeAddressFamilyIPv4,
