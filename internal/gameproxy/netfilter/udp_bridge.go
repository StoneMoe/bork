...
func (bridge *Bridge) udpCreated(event nativeUDPCreatedEvent) {
	...
	cause := intercept.ErrInvalidFlow
	if endpoint != nil || pending {
		cause = intercept.ErrDuplicateFlow
	}
	failure := &intercept.FlowError{NativeID: event.ID, Operation: "create native UDP", Cause: cause}
	if endpoint != nil {
		bridge.recordCallbackError(errors.Join(failure, endpoint.Reset(failure)))
		return
	}
	bridge.recordCallbackError(errors.Join(failure, bridge.backend.SuspendUDP(event.ID)))
}
...
func (bridge *Bridge) udpSend(event nativeUDPSendEvent) {
	...
	for {
		...
		if endpoint != nil {
			...
			bridge.enqueueUDP(endpoint, event.Remote, event.Payload, event.Options)
			return
		}
		...
		if !validUDPSendEvent(event) {
			...
			bridge.finishUDPAdmissionLocked(event.ID, bridge.udpAdmissions[event.ID])
			bridge.mu.Unlock()
			bridge.rejectUDPEndpoint(nil, event.ID, intercept.ErrInvalidFlow)
			return
		}
		if !exists {
			bridge.mu.Unlock()
...
func (bridge *Bridge) enqueueUDP(
	endpoint *udpEndpoint,
	remote netip.AddrPort,
	payload []byte,
	options nativeUDPOptions,
) {
	err := endpoint.enqueue(remote, payload, options)
...
func (bridge *Bridge) rejectUDPEndpoint(endpoint *udpEndpoint, id intercept.NativeID, cause error) {
	...
	bridge.recordCallbackError(errors.Join(failure, bridge.backend.SuspendUDP(id)))
}
...
func validUDPCreatedEvent(event nativeUDPCreatedEvent) bool {
	...
	return (placeholder || validIPv4Endpoint(event.Local)) && pathValid
}
...
func validUDPSendEvent(event nativeUDPSendEvent) bool {
	return validIPv4Endpoint(event.Local) && validUDPDatagram(event.Remote, event.Payload) &&
		len(event.Options.data) <= nativeUDPMaxOptions
}
