...
func TestExactRules_accepts_path_with_exactly_259_UTF16_units(t *testing.T) {
	...
	if rules[0].protocol != nativeProtocolUDP || rules[0].flags != nativeFlagAllow || rules[0].processID == 0 || rules[0].executablePath != "" {
		t.Fatalf("first rule = %#v, want current-process UDP allow", rules[0])
	}
}
