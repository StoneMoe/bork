...
func newUDPEndpoint(
	...
	return &udpEndpoint{
		...
		options: nativeUDPOptions{flags: options.flags, data: slices.Clone(options.data)},
	}
}
...
func (endpoint *udpEndpoint) WriteDatagram(ctx context.Context, datagram intercept.Datagram) error {
	...
	options := endpoint.optionsSnapshot()
	...
	err := endpoint.backend.PostUDPReceive(
		operationCtx, endpoint.metadata.NativeID, datagram.Metadata.OriginalRemote,
		slices.Clone(datagram.Payload), options,
	)
	if err == nil {
		return nil
	}
...
func (endpoint *udpEndpoint) enqueue(remote netip.AddrPort, payload []byte, options nativeUDPOptions) error {
	if !validUDPDatagram(remote, payload) || len(options.data) > nativeUDPMaxOptions {
	...
	case endpoint.incoming <- datagram:
		endpoint.options = nativeUDPOptions{flags: options.flags, data: slices.Clone(options.data)}
		return nil
	default:
...
func (endpoint *udpEndpoint) nativeFailed(cause error) bool {
	endpoint.stateMu.Lock()
	defer endpoint.stateMu.Unlock()
	if endpoint.terminal != nil || endpoint.nativeEnded {
		return false
...
func (endpoint *udpEndpoint) optionsSnapshot() nativeUDPOptions {
	endpoint.stateMu.Lock()
	defer endpoint.stateMu.Unlock()
	return nativeUDPOptions{flags: endpoint.options.flags, data: slices.Clone(endpoint.options.data)}
}
...
func (endpoint *udpEndpoint) validateWrite(datagram intercept.Datagram) error {
	...
	fixed := metadata.Generation == endpoint.metadata.Generation &&
		...
		metadata.ExecutablePath == endpoint.metadata.ExecutablePath &&
		metadata.NativeRuleMatched == endpoint.metadata.NativeRuleMatched &&
		metadata.OriginalLocal == endpoint.metadata.OriginalLocal
