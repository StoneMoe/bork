...
func (bridge *Bridge) finishUDPAdmission(admission *udpAdmission, state intercept.GenerationState) {
	...
	if endpoint := bridge.udpEndpoints[send.ID]; endpoint != nil {
		...
		bridge.enqueueUDP(endpoint, send.Remote, send.Payload, send.Options)
		return
	}
	...
	if !state.Ready {
		...
		bridge.finishUDPAdmissionLocked(send.ID, admission)
		bridge.mu.Unlock()
		failure := &intercept.FlowError{NativeID: send.ID, Operation: "admit native UDP", Cause: intercept.ErrNotReady}
		bridge.recordCallbackError(errors.Join(failure, bridge.backend.SuspendUDP(send.ID)))
		return
	}
	metadata := intercept.Metadata{
		Generation: state.Generation, NativeID: send.ID, ProcessID: admission.socket.PID,
		ExecutablePath: admission.socket.ExecutablePath, NativeRuleMatched: true,
		OriginalLocal: send.Local, OriginalRemote: send.Remote,
	}
	callbacks := bridge.callbacks
	ctx := bridge.startCtx
	endpoint := newUDPEndpoint(ctx, bridge.backend, metadata, send.Options)
	bridge.udpEndpoints[send.ID] = endpoint
	delete(bridge.udpSockets, send.ID)
	enqueueErr := endpoint.enqueue(send.Remote, send.Payload, send.Options)
	bridge.finishUDPAdmissionLocked(send.ID, admission)
	bridge.mu.Unlock()

	if enqueueErr != nil {
		failure := &intercept.FlowError{NativeID: send.ID, Operation: "queue initial native UDP", Cause: enqueueErr}
		bridge.recordCallbackError(errors.Join(failure, endpoint.Reset(failure)))
		return
	}
	go bridge.invokeUDPCallback(callbacks, ctx, endpoint)
}
