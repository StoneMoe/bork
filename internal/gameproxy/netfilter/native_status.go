...
func isBackendFatalStatus(status nativeStatus) bool {
	return status == nativeStatusNotInitialized || status == nativeStatusIOError || status == nativeStatusRebootRequired
}
