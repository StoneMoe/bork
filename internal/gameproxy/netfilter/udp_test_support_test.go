...
type udpTestBackend struct {
	...
	postErr        error
	postHook       func(context.Context, intercept.NativeID, netip.AddrPort, []byte, nativeUDPOptions) error
...
func (*udpTestBackend) UpdateRules(context.Context, []nativeRule) error               { return nil }
...
func (backend *udpTestBackend) PostUDPReceive(
	...
	backend.mu.Lock()
	backend.receives = append(backend.receives, udpReceiveCall{
		id: id, source: source, payload: slices.Clone(payload),
	...
	if hook != nil {
		return hook(ctx, id, source, payload, options)
	}
	return failure
}
...
func (callbacks *udpTestCallbacks) EndpointError(err error) {
	callbacks.mu.Lock()
	errors := callbacks.endpointErrors
	callbacks.mu.Unlock()
	if errors != nil {
		errors <- err
	}
...
func testNativeUDPOptions() nativeUDPOptions {
	return nativeUDPOptions{flags: 0x20, data: []byte{1, 2, 3, 4}}
}
