...
func (backend *sdkNativeBackend) PostTCPReceive(ctx context.Context, id intercept.NativeID, payload []byte) error {
	...
	return backend.operationStatusError(nativeOperationPostTCPReceive, backend.sdk.PostTCPReceive(backend.token, id, payload))
}
...
func (backend *sdkNativeBackend) CloseTCP(id intercept.NativeID) error {
	...
	defer backend.operations.Done()
	return backend.operationStatusError(nativeOperationCloseTCP, backend.sdk.CloseTCP(backend.token, id))
}

func (backend *sdkNativeBackend) PostUDPReceive(
	ctx context.Context,
	...
	if len(options.data) > nativeUDPMaxOptions {
		return fmt.Errorf("UDP options length %d: %w", len(options.data), ErrInvalidNativeAddress)
	}
	if err := backend.beginOperation(ctx); err != nil {
		return err
	}
	defer backend.operations.Done()
	return backend.operationStatusError(
		nativeOperationPostUDPReceive,
		backend.sdk.PostUDPReceive(backend.token, id, source, payload, options),
	)
}
...
func (backend *sdkNativeBackend) SuspendUDP(id intercept.NativeID) error {
	...
	defer backend.operations.Done()
	return backend.operationStatusError(nativeOperationSuspendUDP, backend.sdk.SuspendUDP(backend.token, id))
}
