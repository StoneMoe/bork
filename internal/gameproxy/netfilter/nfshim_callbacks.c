...
int bork_copy_tcp_info(const NF_TCP_CONN_INFO *packed, uint32_t *filtering, DWORD *pid, uint8_t *direction,
    ...
    memcpy(filtering, bytes + offsetof(NF_TCP_CONN_INFO, filteringFlag), sizeof(*filtering));
...
int bork_process_path(uint64_t token, DWORD pid, char *utf8, int capacity, int *length) {
    ...
    if (!api.get_process_name(pid, wide, MAX_PATH)) {
        memset(wide, 0, sizeof(wide));
        if (!api.get_process_name_kernel(pid, wide, MAX_PATH)) return 0;
    }
    int units = 0;
...
void bork_fatal_tcp(uint64_t token, int event, uint64_t id, int reason, int32_t status) {
    ...
    if (bork_api_snapshot(token, &api)) {
        cleanup = reason == BORK_REASON_IPV6 ? api.tcp_disable_filtering(id) : api.tcp_close(id);
    }
...
void bork_fatal_udp(uint64_t token, int event, uint64_t id, int reason, int32_t status) {
    ...
    int32_t cleanup = NF_STATUS_NOT_INITIALIZED;
    if (bork_api_snapshot(token, &api)) {
        cleanup = reason == BORK_REASON_IPV6 ? api.udp_disable_filtering(id) : api.udp_state(id, 1);
    }
    goNFFatal(token, event, id, reason, status, cleanup);
}
