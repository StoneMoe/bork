...
_Static_assert(offsetof(NF_UDP_OPTIONS, options) == 8, "unexpected NF_UDP_OPTIONS ABI");
...
enum bork_nf_reason {
    ...
    BORK_REASON_POST_RECEIVE,
    BORK_REASON_POST_SEND,
    BORK_REASON_SELF_INTERCEPTION,
    BORK_REASON_LOCAL_ROUTE
};
...
typedef NF_STATUS (NFAPI_CC *bork_tcp_post_send_fn)(ENDPOINT_ID, const char *, int);
...
typedef NF_STATUS (NFAPI_CC *bork_tcp_disable_filtering_fn)(ENDPOINT_ID);
...
typedef NF_STATUS (NFAPI_CC *bork_udp_disable_filtering_fn)(ENDPOINT_ID);
...
typedef void (NFAPI_CC *bork_adjust_process_privileges_fn)(void);
...
typedef struct bork_nf_api {
    ...
    bork_tcp_post_send_fn tcp_post_send;
    bork_tcp_post_receive_fn tcp_post_receive;
    bork_tcp_close_fn tcp_close;
    bork_tcp_disable_filtering_fn tcp_disable_filtering;
    bork_udp_post_receive_fn udp_post_receive;
    bork_udp_state_fn udp_state;
    bork_udp_disable_filtering_fn udp_disable_filtering;
    bork_get_udp_info_fn get_udp_info;
    bork_get_process_name_fn get_process_name;
    bork_get_process_name_fn get_process_name_kernel;
    bork_adjust_process_privileges_fn adjust_process_privileges;
} bork_nf_api;
...
int bork_copy_tcp_info(const NF_TCP_CONN_INFO *packed, uint32_t *filtering, DWORD *pid, uint8_t *direction,
