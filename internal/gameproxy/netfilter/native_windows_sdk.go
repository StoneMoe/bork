...
/*
#cgo CFLAGS: -I${SRCDIR}/sdk/nfsdk/include
#cgo LDFLAGS: -lws2_32
...
static inline int bork_nf_rule_set_string(bork_nf_rules *rules, int index,
        ...
        uint32_t process_id, _GoString_ path) {
    return bork_nf_rule_set(rules, index, protocol, direction, family, flags,
                             process_id, _GoStringPtr(path), (int)_GoStringLen(path));
}
...
static inline int32_t bork_nf_udp_post_receive_string(uint64_t token, uint64_t id,
        ...
        _GoString_ payload, uint32_t option_flags, _GoString_ option_data) {
    return bork_nf_udp_post_receive(token, id, a, b, c, d, port,
                                    _GoStringPtr(payload), (int)_GoStringLen(payload),
                                    option_flags, _GoStringPtr(option_data), (int)_GoStringLen(option_data));
}
*/
import "C"
...
func nativeRulesToC(rules []nativeRule) (*C.bork_nf_rules, error) {
	nativeRules := C.bork_nf_rules_new(C.int(len(rules)))
	if nativeRules == nil {
		return nil, errors.New("allocate native rules")
	}
	for index, rule := range rules {
		if C.bork_nf_rule_set_string(
			nativeRules, C.int(index), C.int(rule.protocol), C.uint8_t(rule.direction),
			C.uint16_t(rule.family), C.uint32_t(rule.flags), C.uint32_t(rule.processID), rule.executablePath,
		) == 0 {
			C.bork_nf_rules_free(nativeRules)
			return nil, errors.New("encode native rule")
		}
	}
	return nativeRules, nil
}
...
func (cgoNativeSDK) Start(token uint64, config nativeConfig, rules []nativeRule) nativeStartResult {
	nativeRules, err := nativeRulesToC(rules)
	if err != nil {
...
func (cgoNativeSDK) SetRules(token uint64, rules []nativeRule) nativeStatus {
	nativeRules, err := nativeRulesToC(rules)
	if err != nil {
		return nativeStatusFail
	}
	defer C.bork_nf_rules_free(nativeRules)
	return nativeStatus(C.bork_nf_apply_rules(C.uint64_t(token), nativeRules))
}
...
func (cgoNativeSDK) PostUDPReceive(
	token uint64,
	...
	return nativeStatus(C.bork_nf_udp_post_receive_string(
		...
		C.uint16_t(source.Port()), string(payload), C.uint32_t(options.flags), string(options.data),
	))
}
