...
int bork_nf_rule_set(bork_nf_rules *rules, int index, int protocol,
                     ...
                     uint32_t process_id, const char *path, int path_len) {
    if (rules == NULL || index < 0 || index >= rules->count) return 0;
    int units = 0;
    if (path_len > 0) {
        if (path == NULL) return 0;
        units = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, path, path_len, NULL, 0);
...
int32_t bork_nf_apply_rules(uint64_t token, const bork_nf_rules *rules) {
    bork_nf_api api;
