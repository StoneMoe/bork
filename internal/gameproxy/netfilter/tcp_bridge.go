...
func (bridge *Bridge) tcpConnected(event nativeTCPConnectedEvent) {
	...
	bridge.invokeTCPCallback(callbacks, ctx, flow)
}
...
func (bridge *Bridge) tcpClosed(id intercept.NativeID) {
	bridge.mu.Lock()
	if pending := bridge.tcpPending[id]; pending != nil {
		pending.nativeClosed = true
		_ = pending.listener.SetDeadline(time.Now().Add(tcpRedirectClosedGrace))
	}
	flow := bridge.flows[id]
	if flow != nil {
