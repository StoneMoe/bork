...
func TestNewBridge_builds_exact_outbound_IPv4_rules_in_path_order(t *testing.T) {
	...
	want := []nativeRule{
		{direction: nativeDirectionOutbound, family: nativeAddressFamilyIPv4, protocol: nativeProtocolUDP, flags: nativeFlagAllow, processID: intercept.ProcessID(os.Getpid())},
		{direction: nativeDirectionOutbound, family: nativeAddressFamilyIPv4, protocol: nativeProtocolTCP, flags: nativeFlagIndicateConnectRequests, executablePath: `c:\games\alpha.exe`},
		...
		{direction: nativeDirectionOutbound, family: nativeAddressFamilyIPv4, protocol: nativeProtocolTCP, flags: nativeFlagIndicateConnectRequests, executablePath: `\\server\share\bravo.exe`},
...
func TestNativeRuleConstants_match_verified_SDK_header_values(t *testing.T) {
	if nativeDirectionOutbound != 2 || nativeAddressFamilyIPv4 != 2 || nativeFlagAllow != 0 || nativeFlagFilter != 2 || nativeFlagIndicateConnectRequests != 16 {
		t.Fatalf("native constants = direction %d, family %d, filter %d, connect requests %d", nativeDirectionOutbound, nativeAddressFamilyIPv4, nativeFlagFilter, nativeFlagIndicateConnectRequests)
	}
...
func (callbackStub) EndpointError(error)                                    {}
