...
func (backend *sdkNativeBackend) UpdateRules(ctx context.Context, rules []nativeRule) error {
	if err := backend.beginOperation(ctx); err != nil {
		return err
	}
	defer backend.operations.Done()
	return backend.operationStatusError(nativeOperationSetRules, backend.sdk.SetRules(backend.token, rules))
}

func (backend *sdkNativeBackend) operationStatusError(operation nativeOperation, status nativeStatus) error {
	err := nativeStatusError(operation, status)
	if err != nil && isBackendFatalStatus(status) {
		backend.reportFatal(err)
	}
	return err
}
