...
//export goNFTCPConnectRequest
func goNFTCPConnectRequest(token, id C.uint64_t, pid C.uint32_t, path *C.char, pathLength C.int,
	...
	dispatchNativeCallback(uint64(token), nativeEventTCPConnectRequest, func(owner *sdkNativeBackend) {
		redirectPort = owner.deliverTCPConnectRequest(nativeTCPConnectRequestEvent{
	...
	return C.uint16_t(redirectPort)
}
...
func goNFUDPSend(token, id C.uint64_t,
	localA, localB, localC, localD C.uint8_t, localPort C.uint16_t,
	remoteA, remoteB, remoteC, remoteD C.uint8_t, remotePort C.uint16_t,
	optionFlags C.uint32_t, optionData *C.char, optionLength C.int,
	...
	dispatchNativeCallback(uint64(token), nativeEventUDPSend, func(owner *sdkNativeBackend) {
		owner.deliverUDPSend(nativeUDPSendEvent{
			...
			Options: nativeUDPOptions{flags: uint32(optionFlags), data: []byte(C.GoStringN(optionData, optionLength))},
		})
	})
}
...
func goNFFatal(token C.uint64_t, event C.int, id C.uint64_t, reason C.int,
	...
	dispatchNativeCallback(uint64(token), nativeEvent(event), func(owner *sdkNativeBackend) {
		...
		owner.deliverEndpointError(&NativeCallbackError{
