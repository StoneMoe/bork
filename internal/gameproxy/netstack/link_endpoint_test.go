...
type reentrantDispatcher struct{ endpoint *LinkEndpoint }

func (d *reentrantDispatcher) DeliverNetworkPacket(tcpip.NetworkProtocolNumber, *stack.PacketBuffer) {
	d.endpoint.SetMTU(d.endpoint.MTU())
	d.endpoint.Close()
}

func (*reentrantDispatcher) DeliverLinkPacket(tcpip.NetworkProtocolNumber, *stack.PacketBuffer) {}
...
func TestLinkEndpoint_accepts_inbound_packet_above_outbound_MTU(t *testing.T) {
	endpoint, err := NewLinkEndpoint(4, 20, 1)
	if err != nil {
		t.Fatalf("NewLinkEndpoint: %v", err)
	}
	defer endpoint.Close()
	...
	if len(dispatcher.packet) != len(packet) {
		t.Fatalf("delivered packet length = %d, want %d", len(dispatcher.packet), len(packet))
	}
}

func TestLinkEndpoint_trims_inbound_IPv4_padding(t *testing.T) {
	endpoint, err := NewLinkEndpoint(4, 1500, 1)
	if err != nil {
		t.Fatalf("NewLinkEndpoint: %v", err)
	}
	...
	dispatcher := &captureDispatcher{}
	endpoint.Attach(dispatcher)
	packet := validIPv4Packet(46)
	binary.BigEndian.PutUint16(packet[2:4], 36)

	if err := endpoint.InjectInbound(4, packet); err != nil {
		t.Fatalf("InjectInbound: %v", err)
	}
	if len(dispatcher.packet) != 36 {
		t.Fatalf("delivered packet length = %d, want IPv4 total length 36", len(dispatcher.packet))
	}
}

func TestLinkEndpoint_marks_inbound_checksums_validated(t *testing.T) {
	endpoint, err := NewLinkEndpoint(1, 1500, 1)
	if err != nil {
		t.Fatal(err)
	}
	defer endpoint.Close()
	if capabilities := endpoint.Capabilities(); capabilities&stack.CapabilityRXChecksumOffload == 0 {
		t.Fatalf("link capabilities = %d, want RX checksum offload", capabilities)
	}
}
...
func TestLinkEndpointAllowsReentrantDelivery(t *testing.T) {
	endpoint, err := NewLinkEndpoint(1, 1500, 1)
	if err != nil {
		t.Fatal(err)
	}
	endpoint.Attach(&reentrantDispatcher{endpoint: endpoint})
	result := make(chan error, 1)
	go func() { result <- endpoint.InjectInbound(1, validIPv4Packet(20)) }()
	select {
	case err := <-result:
		if err != nil {
			t.Fatal(err)
	...
	case <-time.After(time.Second):
		t.Fatal("inbound delivery held the link mutex across a reentrant callback")
	}
	if endpoint.IsAttached() {
		t.Fatal("dispatcher could not close the link during delivery")
	}
}
