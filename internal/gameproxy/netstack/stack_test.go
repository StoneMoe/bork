//go:build game_proxy

package netstack

import (
	"context"
	"encoding/binary"
	"errors"
	"io"
	"net"
	"net/netip"
	"testing"
	"time"

	"github.com/sagernet/gvisor/pkg/tcpip"
	"github.com/sagernet/gvisor/pkg/tcpip/adapters/gonet"
	"github.com/sagernet/gvisor/pkg/tcpip/header"
)

func TestNewConfiguresOneIPv4NICWithSlash24AndDefaultRoute(t *testing.T) {
	network, err := New(Config{Address: netip.MustParseAddr("10.20.30.40"), MTU: 1400, QueueSize: 8, Generation: 2})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	defer network.Close()

	nics := network.stack.NICInfo()
	info, ok := nics[stackNICID]
	if len(nics) != 1 || !ok {
		t.Fatalf("NICInfo = %#v, want one NIC %d", nics, stackNICID)
	}
	if len(info.ProtocolAddresses) != 1 || info.ProtocolAddresses[0].Protocol != header.IPv4ProtocolNumber {
		t.Fatalf("protocol addresses = %#v, want one IPv4 address", info.ProtocolAddresses)
	}
	address := info.ProtocolAddresses[0].AddressWithPrefix
	if address.PrefixLen != 24 || address.Address.String() != "10.20.30.40" {
		t.Fatalf("address = %v/%d, want 10.20.30.40/24", address.Address, address.PrefixLen)
	}
	routes := network.stack.GetRouteTable()
	if len(routes) != 1 || routes[0].NIC != stackNICID || routes[0].Destination.Prefix() != 0 {
		t.Fatalf("routes = %#v, want one IPv4 default route", routes)
	}
}

func TestOpenUDPSendsToMultipleRemoteDestinations(t *testing.T) {
	client := newTestStack(t, "10.0.0.1", 9)
	server := newTestStack(t, "10.0.0.2", 9)
	stopLinks := connectTestStacks(t, client, server)
	defer stopLinks()

	first := openTestUDP(t, server, 9001)
	defer first.Close()
	second := openTestUDP(t, server, 9002)
	defer second.Close()
	packetConn, err := client.OpenUDP()
	if err != nil {
		t.Fatalf("OpenUDP: %v", err)
	}
	defer packetConn.Close()

	writeUDP(t, packetConn, "first", "10.0.0.2", 9001)
	writeUDP(t, packetConn, "second", "10.0.0.2", 9002)
	readUDP(t, first, "first")
	readUDP(t, second, "second")
}

func TestNewEnforcesIPv4MinimumMTU(t *testing.T) {
	for _, mtu := range []uint32{0, 46, 67, 68} {
		network, err := New(Config{Address: netip.MustParseAddr("10.0.0.1"), MTU: mtu, QueueSize: 1})
		if network != nil {
			network.Close()
		}
		if mtu < 68 && !errors.Is(err, ErrInvalidConfig) {
			t.Fatalf("New(MTU=%d) error = %v, want ErrInvalidConfig", mtu, err)
		}
		if mtu == 68 && err != nil {
			t.Fatalf("New(MTU=68): %v", err)
		}
	}
}

func TestDialTCPPreservesHalfClose(t *testing.T) {
	client := newTestStack(t, "10.0.0.1", 1)
	server := newTestStack(t, "10.0.0.2", 1)
	stopLinks := connectTestStacks(t, client, server)
	defer stopLinks()
	listener, err := gonet.ListenTCP(server.stack, tcpip.FullAddress{
		NIC: stackNICID, Addr: server.address, Port: 9000,
	}, header.IPv4ProtocolNumber)
	if err != nil {
		t.Fatal(err)
	}
	defer listener.Close()
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	connection, err := client.DialTCP(ctx, netip.MustParseAddrPort("10.0.0.2:9000"))
	if err != nil {
		t.Fatal(err)
	}
	defer connection.Close()
	halfClose, ok := connection.(interface {
		CloseRead() error
		CloseWrite() error
	})
	if !ok {
		t.Fatalf("DialTCP returned %T without half-close support", connection)
	}
	peer, err := listener.Accept()
	if err != nil {
		t.Fatal(err)
	}
	defer peer.Close()
	for _, conn := range []net.Conn{connection, peer} {
		if err := conn.SetDeadline(time.Now().Add(2 * time.Second)); err != nil {
			t.Fatal(err)
		}
	}
	if _, err := connection.Write([]byte("request")); err != nil {
		t.Fatal(err)
	}
	if err := halfClose.CloseWrite(); err != nil {
		t.Fatal(err)
	}
	if payload, err := io.ReadAll(peer); err != nil || string(payload) != "request" {
		t.Fatalf("peer read = %q, %v, want request followed by EOF", payload, err)
	}
	if _, err := peer.Write([]byte("response")); err != nil {
		t.Fatal(err)
	}
	if err := peer.(*gonet.TCPConn).CloseWrite(); err != nil {
		t.Fatal(err)
	}
	if payload, err := io.ReadAll(connection); err != nil || string(payload) != "response" {
		t.Fatalf("client read = %q, %v, want response followed by EOF", payload, err)
	}
}

func TestOpenUDPAllocatesDistinctAssignedAddressEndpoints(t *testing.T) {
	network := newTestStack(t, "10.0.0.1", 9)
	const endpointCount = 33
	connections := make([]net.PacketConn, 0, endpointCount)
	ports := make(map[uint16]struct{}, endpointCount)

	for index := range endpointCount {
		connection, err := network.OpenUDP()
		if err != nil {
			t.Fatalf("OpenUDP %d: %v", index, err)
		}
		connections = append(connections, connection)
		t.Cleanup(func() { _ = connection.Close() })
		local, ok := connection.LocalAddr().(*net.UDPAddr)
		if !ok || !local.IP.Equal(net.IPv4(10, 0, 0, 1)) || local.Port == 0 {
			t.Fatalf("OpenUDP %d local address = %v, want assigned address and allocated port", index, connection.LocalAddr())
		}

		remote := &net.UDPAddr{IP: net.IPv4(198, 51, 100, byte(index+1)), Port: 7770}
		if _, err := connection.WriteTo([]byte("12345678"), remote); err != nil {
			t.Fatalf("WriteTo %d: %v", index, err)
		}
		packet, err := network.LinkEndpoint().ReadPacket(context.Background())
		if err != nil {
			t.Fatalf("ReadPacket %d: %v", index, err)
		}
		if got := netip.AddrFrom4([4]byte(packet.Data[12:16])); got != netip.MustParseAddr("10.0.0.1") {
			t.Fatalf("packet %d source = %v, want assigned address", index, got)
		}
		port := binary.BigEndian.Uint16(packet.Data[20:22])
		if port == 0 {
			t.Fatalf("packet %d source port is zero", index)
		}
		if _, duplicate := ports[port]; duplicate {
			t.Fatalf("packet %d reused UDP source port %d", index, port)
		}
		ports[port] = struct{}{}
	}
}

func TestDialTCPReturnsContextCancellation(t *testing.T) {
	network := newTestStack(t, "10.0.0.1", 1)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	_, err := network.DialTCP(ctx, netip.MustParseAddrPort("10.0.0.2:80"))
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("DialTCP error = %v, want context.Canceled", err)
	}
}

func TestCloseUnblocksTCPDialAndUDPRead(t *testing.T) {
	network := newTestStack(t, "10.0.0.1", 1)
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	packetConn, err := network.OpenUDP()
	if err != nil {
		t.Fatalf("OpenUDP: %v", err)
	}
	udpResult := make(chan error, 1)
	go func() {
		_, _, readErr := packetConn.ReadFrom(make([]byte, 16))
		udpResult <- readErr
	}()
	tcpResult := make(chan error, 1)
	go func() {
		_, dialErr := network.DialTCP(ctx, netip.MustParseAddrPort("10.0.0.2:80"))
		tcpResult <- dialErr
	}()

	readCtx, stopRead := context.WithTimeout(ctx, time.Second)
	defer stopRead()
	if _, err := network.LinkEndpoint().ReadPacket(readCtx); err != nil {
		t.Fatalf("wait for pending TCP SYN: %v", err)
	}
	closed := make(chan struct{})
	go func() {
		network.Close()
		close(closed)
	}()
	select {
	case <-closed:
	case <-time.After(2 * time.Second):
		t.Fatal("stack close did not cancel the pending dial")
	}
	network.Close()
	if err := <-udpResult; err == nil {
		t.Fatal("UDP ReadFrom returned nil after stack close")
	}
	if err := <-tcpResult; !errors.Is(err, net.ErrClosed) {
		t.Fatalf("TCP Dial error = %v, want net.ErrClosed", err)
	}
	if ctx.Err() != nil {
		t.Fatalf("stack close canceled the caller context: %v", ctx.Err())
	}
	if _, err := network.OpenUDP(); !errors.Is(err, net.ErrClosed) {
		t.Fatalf("OpenUDP after close = %v, want net.ErrClosed", err)
	}
}

func newTestStack(t *testing.T, address string, generation uint64) *Stack {
	t.Helper()
	network, err := New(Config{Address: netip.MustParseAddr(address), MTU: 1500, QueueSize: 64, Generation: generation})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	t.Cleanup(network.Close)
	return network
}

func openTestUDP(t *testing.T, network *Stack, port uint16) net.PacketConn {
	t.Helper()
	address := tcpip.FullAddress{NIC: stackNICID, Addr: network.address, Port: port}
	conn, err := gonet.DialUDP(network.stack, &address, nil, header.IPv4ProtocolNumber)
	if err != nil {
		t.Fatalf("DialUDP bind: %v", err)
	}
	return conn
}

func writeUDP(t *testing.T, conn net.PacketConn, payload, address string, port int) {
	t.Helper()
	if err := conn.SetWriteDeadline(time.Now().Add(2 * time.Second)); err != nil {
		t.Fatalf("SetWriteDeadline: %v", err)
	}
	written, err := conn.WriteTo([]byte(payload), &net.UDPAddr{IP: net.ParseIP(address), Port: port})
	if err != nil || written != len(payload) {
		t.Fatalf("WriteTo = (%d, %v), want (%d, nil)", written, err, len(payload))
	}
}

func readUDP(t *testing.T, conn net.PacketConn, expected string) {
	t.Helper()
	if err := conn.SetReadDeadline(time.Now().Add(2 * time.Second)); err != nil {
		t.Fatalf("SetReadDeadline: %v", err)
	}
	buffer := make([]byte, 64)
	read, _, err := conn.ReadFrom(buffer)
	if err != nil {
		t.Fatalf("ReadFrom: %v", err)
	}
	if string(buffer[:read]) != expected {
		t.Fatalf("payload = %q, want %q", buffer[:read], expected)
	}
}
