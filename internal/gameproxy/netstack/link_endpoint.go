...
func (endpoint *LinkEndpoint) InjectInbound(generation uint64, packet []byte) error {
	...
	packet = packet[:ipv4Header.TotalLength()]
	...
	defer owned.DecRef()
	// Delivery may synchronously call back into MTU or WritePackets.
	dispatcher.DeliverNetworkPacket(header.IPv4ProtocolNumber, owned)
	return nil
}
