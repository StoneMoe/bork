...
const MinMTU = header.IPv4MinimumMTU
...
func New(config Config) (*Stack, error) {
	if !config.Address.Is4() || config.MTU < MinMTU || config.QueueSize <= 0 {
...
func (network *Stack) DialTCP(ctx context.Context, remote netip.AddrPort) (net.Conn, error) {
	...
	defer cancel()
	stopCancel := context.AfterFunc(network.lifetime, cancel)
	defer stopCancel()
...
func (network *Stack) OpenUDP() (net.PacketConn, error) {
	...
	defer network.operations.Done()
	local := tcpip.FullAddress{NIC: stackNICID, Addr: network.address}
	connection, err := gonet.DialUDP(network.stack, &local, nil, header.IPv4ProtocolNumber)
	if err != nil {
...
func (network *Stack) Close() {
	network.closeOnce.Do(func() {
		...
		network.link.Close()
		// Dials must finish registering or closing endpoints before gVisor snapshots them.
