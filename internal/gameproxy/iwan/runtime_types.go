...
const (
	DataPathEventStart DataPathEventPhase = "generation-start"
	DataPathEventEnd   DataPathEventPhase = "generation-end"
)

type DataPathEvent struct {
	Phase  DataPathEventPhase
	Status Status
}
...
func normalizeOptions(options Options) (Options, Credentials, error) {
	...
	if address, err := netip.ParseAddr(options.Node.Server); err == nil {
		address = address.Unmap()
		if !address.Is4() {
			return Options{}, Credentials{}, fmt.Errorf("server must be IPv4: %w", ErrInvalidOptions)
		}
		options.Node.Server = address.String()
	}
	...
	if options.Node.MTU < netstack.MinMTU || options.Node.MTU > MaxMTU {
