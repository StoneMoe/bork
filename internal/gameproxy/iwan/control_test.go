...
func TestParseControl_validatesSignatureTypeAndSession(t *testing.T) {
	...
	if control, err := ParseControl(mismatch, session); err != nil || control.Type != TypeEchoResponse {
		t.Fatalf("ParseControl(mismatched ECHO response) = %#v, %v", control, err)
	}
	closeMismatch := BuildClose(session)
	closeMismatch[2] ^= 1
	signControl(closeMismatch)
	if _, err := ParseControl(closeMismatch, session); !errors.Is(err, ErrSessionMismatch) {
		t.Fatalf("ParseControl(mismatched CLOSE) error = %v, want ErrSessionMismatch", err)
	}
}

func TestParseControl_accepts_short_signed_echo_response(t *testing.T) {
	session := goldenSession(t)
	response := make([]byte, signedHeaderSize+8)
	writeHeader(response, wireHeader{typ: TypeEchoResponse, token: session.Token, session: session.ID})
	binary.LittleEndian.PutUint64(response[signedHeaderSize:], uint64(time.Unix(10, 0).UnixMicro()))
	signControl(response)

	control, err := ParseControl(response, session)
	if err != nil {
		t.Fatal(err)
	}
	if control.Type != TypeEchoResponse || !control.Echo.Timestamp.Equal(time.Unix(10, 0)) {
		t.Fatalf("ParseControl() = %#v", control)
	}
}
