...
func (supervisor *Supervisor) retry(id uint64, dataPath *dataPathCounters, err error) {
	supervisor.mu.Lock()
	defer supervisor.mu.Unlock()
...
func (supervisor *Supervisor) fail(id uint64, dataPath *dataPathCounters, err error) {
	supervisor.mu.Lock()
	defer supervisor.mu.Unlock()
	if supervisor.status.Generation != id {
		return
