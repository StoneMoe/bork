...
func TestBuildOpen_matchesPinnedPlaintextGoldenVector(t *testing.T) {
	credentials, err := NewCredentials(" myuser ", "mypassword")
	if err != nil {
		t.Fatal(err)
	}

	packet, err := BuildOpen(credentials, DefaultMTU, false)
	if err != nil {
		t.Fatal(err)
	}

	want := mustDecodeHex(t, "130000000000000039c6b588392392701064efe9e0dd4e980304057801086d797573657202122712fab82fc2a7d17c8974e03e25be80")
	if !bytes.Equal(packet, want) {
		t.Fatalf("BuildOpen() = %x, want %x", packet, want)
	}
}

func TestBuildOpen_matchesPinnedEncryptedGoldenVector(t *testing.T) {
	credentials, err := NewCredentials("myuser", "mypassword")
	if err != nil {
		t.Fatal(err)
	}
	packet, err := BuildOpen(credentials, DefaultMTU, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseOpen_returnsTypedAuthenticationFields(t *testing.T) {
	packet := mustDecodeHex(t, "130000000000000039c6b588392392701064efe9e0dd4e980304057801086d797573657202122712fab82fc2a7d17c8974e03e25be80")
	...
	if request.Username != "myuser" || request.MTU != 1400 || request.Encrypt {
...
func TestParseOpen_rejectsMalformedTLVAndUnknownCriticalTag(t *testing.T) {
	valid := mustDecodeHex(t, "130000000000000039c6b588392392701064efe9e0dd4e980304057801086d797573657202122712fab82fc2a7d17c8974e03e25be80")
	tests := [][]byte{
