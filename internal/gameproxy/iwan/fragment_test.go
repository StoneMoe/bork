...
func TestReassembler_acceptsReferenceIdentityAndIgnoresReservedMetadata(t *testing.T) {
	session := goldenSession(t)
	packet := fragmentPacket(session, 1, 0, false, []byte("x"))
	reassembler := NewReassembler()

	packet[2] ^= 1
	if _, complete, err := reassembler.Push(packet, session, time.Unix(100, 0)); err != nil || complete {
		t.Fatalf("Push() with mismatched identity = complete %v, error %v", complete, err)
	}
	packet = fragmentPacket(session, 1, 0, false, []byte("x"))
	bitfield := binary.LittleEndian.Uint32(packet[20:24]) | 2
	binary.LittleEndian.PutUint32(packet[20:24], bitfield)
	if _, complete, err := reassembler.Push(packet, session, time.Unix(100, 0)); err != nil || complete {
		t.Fatalf("Push() with reserved metadata = complete %v, error %v", complete, err)
	}
	encrypted := []byte("packet")
	xorBytes(session.xorKey, encrypted)
	zeroID := fragmentPacket(session, 0, 0, true, encrypted)
	if payload, complete, err := reassembler.Push(zeroID, session, time.Unix(100, 0)); err != nil || !complete || string(payload) != "packet" {
		t.Fatalf("Push() with zero ID = %q, complete %v, error %v", payload, complete, err)
	}
}

func TestReassembler_ignoresIdenticalDuplicateFragment(t *testing.T) {
	session := plaintextGoldenSession(t)
	reassembler := NewReassembler()
	now := time.Unix(100, 0)
	first := fragmentPacket(session, 9, 0, false, []byte("hello "))
	for index := range 2 {
		if payload, complete, err := reassembler.Push(first, session, now.Add(time.Duration(index)*time.Second)); err != nil || complete || payload != nil {
			t.Fatalf("first Push(%d) = %q, %v, %v", index, payload, complete, err)
		}
	}
	payload, complete, err := reassembler.Push(fragmentPacket(session, 9, 6, true, []byte("world")), session, now.Add(2*time.Second))
	if err != nil || !complete || string(payload) != "hello world" {
		t.Fatalf("final Push() = %q, %v, %v", payload, complete, err)
	}
}
