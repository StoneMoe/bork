...
func TestSupervisor_replacesInactiveGenerationAndIgnoresLateOldACK(t *testing.T) {
	...
	lateACK := buildTestACK(ackSpec{token: Token{9, 9}, session: SessionID{9, 9, 9, 9}, mtu: 1200, encrypt: true})
	server.send(t, oldOpen.peer, lateACK)
	currentACK := buildTestACK(ackSpec{token: Token{2, 2}, session: SessionID{2, 2, 2, 2}, mtu: 1300, encrypt: true})
...
func TestSupervisor_initialAuthenticationTimeoutFailsWithoutRetry(t *testing.T) {
	server := newFakeReferenceServer(t)
	supervisor, err := newSupervisor(testOptions(t, server), testTimings())
	if err != nil {
		t.Fatal(err)
...
func TestSupervisor_startResetsReadinessHistory(t *testing.T) {
	...
	supervisor.mu.Lock()
	everReady := supervisor.everReady
	supervisor.mu.Unlock()
	if everReady {
		t.Fatal("Start retained readiness history from a previous run")
	}
	supervisor.Stop()
}
