...
func TestManager_Start_reaches_running_only_after_dependencies_ready(t *testing.T) {
	...
	if status.State != StateRunning || status.Generation != 7 || status.ExecutableCount != 2 || !reflect.DeepEqual(status.Directories, []string{"games"}) || status.Error != "" {
		t.Fatalf("running status = %#v", status)
	}
	manager.Stop()
}

func TestManager_Start_logs_generation_before_synchronous_bridge_callback(t *testing.T) {
	log := &eventLog{}
	bridge := newFakeBridge(log)
	bridge.startHook = func(callbacks intercept.Callbacks) {
		callbacks.EndpointError(errors.New("early endpoint failure"))
	}
	supervisor := newFakeSupervisor(log, iwan.Status{State: iwan.StateReady, Generation: 7})
	...
	startIndex, endpointIndex := -1, -1
	for index, event := range events {
		if strings.Contains(event.Message, "generation=7 phase=generation-start") {
			startIndex = index
		}
		if strings.Contains(event.Message, "early endpoint failure") {
			endpointIndex = index
		}
	}
	if startIndex < 0 || endpointIndex < 0 || startIndex >= endpointIndex {
