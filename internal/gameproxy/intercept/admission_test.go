...
func TestRelay_match_uses_native_rule_only_when_executable_path_is_missing(t *testing.T) {
	t.Run("trusted missing path", func(t *testing.T) {
		relay := newTestRelay(t, fakeRules{err: errUnexpectedDial}, &fakeDialer{})
		metadata := testMetadata(1, 14, 443)
		metadata.ExecutablePath = ""
		metadata.NativeRuleMatched = true
	...
	t.Run("untrusted missing path", func(t *testing.T) {
		relay := newTestRelay(t, fakeRules{selected: true}, &fakeDialer{})
		metadata := testMetadata(1, 15, 443)
		metadata.ExecutablePath = ""

		if err := relay.match(metadata); !errors.Is(err, ErrUnselected) {
			t.Fatalf("match() error = %v, want ErrUnselected", err)
		}
	})

	t.Run("trusted non-empty path is still matched", func(t *testing.T) {
		relay := newTestRelay(t, fakeRules{selected: false}, &fakeDialer{})
		metadata := testMetadata(1, 16, 443)
		metadata.NativeRuleMatched = true
...
func (dialer *fakeDialer) OpenUDP() (net.PacketConn, error) {
	...
	if len(dialer.packetConns) > 0 {
		packetConn := dialer.packetConns[0]
		dialer.packetConns = dialer.packetConns[1:]
