...
func TestRelay_TCP_rewrites_DNS_and_relays_both_directions(t *testing.T) {
	...
	if traffic.UploadBytes != uint64(len("native payload")) || traffic.DownloadBytes != uint64(len("stack payload")) {
		t.Fatalf("traffic = %#v, want directional TCP payload totals", traffic)
	}
	request, response, complete := <-events, <-events, <-events
	if request.Kind != FlowEventRequest || request.RequestID != "tcp-21" {
		t.Fatalf("TCP request event = %#v", request)
	}
	if response.Kind != FlowEventResponse || response.RequestID != request.RequestID || response.Bytes != uint64(len("stack payload")) {
		t.Fatalf("TCP response event = %#v", response)
	}
	if complete.Kind != FlowEventComplete || complete.RequestID != request.RequestID ||
		complete.UploadBytes != uint64(len("native payload")) || complete.DownloadBytes != uint64(len("stack payload")) {
		t.Fatalf("TCP completion event = %#v", complete)
	}
}
...
func TestRelay_TCP_resets_flow_when_dial_fails(t *testing.T) {
	...
	endpointErrors := make(chan error, 1)
	relay.options.OnFlowEvent = func(event FlowEvent) {
		if event.Kind == FlowEventFailed {
			failedEvents <- event
		}
	}
	relay.options.OnEndpointError = func(err error) { endpointErrors <- err }
	...
	if failed := <-failedEvents; !errors.Is(failed.Err, dialFailure) {
		t.Fatalf("failed event = %#v", failed)
	}
	select {
	case err := <-endpointErrors:
...
func TestRelay_TCP_normal_cancellation_completes_without_endpoint_error(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	nativeRelay, nativeApp := net.Pipe()
	stackRelay, stackServer := net.Pipe()
	flow := newPipeTCP(testMetadata(9, 24, 443), nativeRelay)
	dialer := &fakeDialer{tcpConn: stackRelay, tcpCalled: make(chan struct{})}
	relay := newTestRelay(t, fakeRules{selected: true}, dialer)
	completed := make(chan FlowEvent, 1)
	endpointErrors := make(chan error, 1)
	relay.options.OnFlowEvent = func(event FlowEvent) {
		if event.Kind == FlowEventComplete {
			completed <- event
		}
	}
	relay.options.OnEndpointError = func(err error) { endpointErrors <- err }
	relay.SetState(GenerationState{Generation: 9, Ready: true})
	if err := relay.TCP(ctx, flow); err != nil {
		t.Fatal(err)
	}
	waitDial(t, dialer)
	...
	complete := <-completed
	if complete.Err != nil {
		t.Fatalf("canceled completion error = %v, want nil", complete.Err)
	}
	select {
	case err := <-endpointErrors:
		t.Fatalf("canceled endpoint error = %v", err)
	default:
	}
	_ = nativeApp.Close()
	_ = stackServer.Close()
